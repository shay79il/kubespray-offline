...
# Skip /etc/hosts update when not installing k8s (already configured from previous install)
if [[ "${SKIP_INSTALL}" == "yes" ]]; then
    ./igz_run_ansible.sh -i inventory/igz/igz_inventory.ini igz_pre_install.yml --become --extra-vars=@igz_override.yml --extra-vars skip_hosts_update=true
else
